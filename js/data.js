/**
 * ALICE ESL Phonics P3 - 萌蛙過河單字大冒險題庫核心資料 (data.js)
 * 課本學習範圍：Page 04 - 12
...
const VOCAB_MAP = new Map(P3_VOCABULARY.map(v => [v.id, v]));
...
class QuestionGenerator {
  ...
  static generateListenQuestion() {
    const target = P3_VOCABULARY[Math.floor(Math.random() * P3_VOCABULARY.length)];
    const others = P3_VOCABULARY.filter(v => v.id !== target.id);
  ...
  static generatePhonicsQuestion() {
    ...
    const target = validTargets.length > 0 ? validTargets[Math.floor(Math.random() * validTargets.length)] : P3_VOCABULARY[0];
    
    // 干擾項
    const distractors = P3_VOCABULARY.filter(v => !targetWordIds.includes(v.id));
  ...
  static generateActionQuestion() {
    ...
    const target = VOCAB_MAP.get(qData.correct) || {
      ...
      word: qData.correct,
      zh: qData.correct,
      image: `P3_flashcards_images/P3_${qData.correct}.webp`,
      audioEn: `P3_flashcards_audios/P3_${qData.correct}.mp3`,
      audioZh: `P3_flashcards_audios/P3_${qData.correct}_zh.mp3`
    };
    ...
    const optionItems = qData.options.map(id => VOCAB_MAP.get(id) || {
      ...
      word: id,
      zh: id,
      image: `P3_flashcards_images/P3_${id}.webp`,
      audioEn: `P3_flashcards_audios/P3_${id}.mp3`,
      audioZh: `P3_flashcards_audios/P3_${id}_zh.mp3`
    });
